...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should not allow non-owner to do operation', async () => {
        await this.yfnoToken.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.yfnoToken.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.yfnoToken.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.yfnoToken.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.yfnoToken.owner()).valueOf(), carol);
    });

    it('should also work with YFNOEngine', async () => {
        ...
        await this.YFNOEngine.add('100', this.lp1.address, true);
        await this.YFNOEngine.transferOwnership(this.timelock.address, { from: alice });
        const eta = (await time.latest()).add(time.duration.days(4));
        await this.timelock.queueTransaction(
            this.YFNOEngine.address, '0', 'set(uint256,uint256,bool)',
        ...
        assert.equal((await this.YFNOEngine.poolInfo('0')).valueOf().allocPoint, '200');
        assert.equal((await this.YFNOEngine.totalAllocPoint()).valueOf(), '300');
